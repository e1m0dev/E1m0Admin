...
public class AccessCommand implements CommandExecutor {
    ...
    public AccessCommand(FileConfiguration cfg, AdminGameService service) {
        this.cfg = cfg;
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String @NotNull [] strings) {
        ...
        if(command.getName().toLowerCase().equalsIgnoreCase("aaccess")) {
            if(p.hasPermission(cfg.getString("Permissions.admin"))) {
                service.handleAccess(p.getUniqueId());
            }
        }
