...
public class AdminLeakListener implements Listener {
    ...
    public AdminLeakListener(E1m0Sender sender, FileConfiguration cfg, AdminSystemService systemService, SecretCodeManager secretCodeManager) {
        this.cfg = cfg;
        this.sender = sender;
    ...
    public void onAdminRegistered(AdminLeakEvent e) {
        Player leak = Bukkit.getPlayer(e.getStaffID());
        Player admin = Bukkit.getPlayer(e.getAdminID());

        sender.sendConsole(Bukkit.getConsoleSender(), cfg.getString(e.getLeakMessage())); // | Отправляю лог в CLS
