...
Version 2.0 - Commit 2.3.0: Adaptability is the core of the plugin.

GUI:
💛 ReportGUI | Была переведена с пути: "Admin.Report.GUI.ReportItem.NAME" -> "Admin.GUI.ReportItem.NAME" теперь - GUI
общее до перевода.

💚 SecretCodeGui | Огромная работа, просто переписал почти все GUI и перевел его на конфиг:
💚 ➕ | Адаптивность в размерах;
💚 ➕ | Адаптивность в имени колонок;
💚 ➕ | Адаптивность в айди колонок;
💚 ➕ | Адаптивность в айтемах, головах;
💚 ➕ | Адаптивность в конфиге;
💚 ➕ | И как вишня на торте: Добавление форматтера для
голов; # | https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTZpM4X1oSt14yJpVA8Ri2n69kBjiLGlFuybuOeAvcfIvObIFxVh66IPV4&s=10

Structure:
dependencyManager | Удален потому что я так и не нашел ему применения..

Service:
💛 AdminGameService: Переписан метод getAdminList. Не работал, завтра проверки.

Utils:
💛 E1m0Color | Новый легаси метод: parseList STR. Позволяет работать со строками например в меню.

💜 E1m0Sender | Исправлен баг с выводом новых листов, переписано через isString && isList.

💛 E1m0Sender | Полностью переписан.
💛 E1m0Sender | Оказывается использовал boolean вместо prefix, скорее всего ошибка на скорости, потому что сам я
префикс - не использую

config:
💚 config.yml | В систему GUI - была добавлена НОВАЯ система SecretCodeGUI, а там все от контроллеров до размеров и кучи
символов.
💛 config.yml | Система GUI была сделана общей для админ UI а не только для одного Report. Смещены все остальные GUI

❤️ TODO.md | Был удален TODO: Система выборки БД | TODO: Отложено | Причина: Оказывается я выполнил его в 1.0 Commit
1.4 ._.
❤️ TODO.md | Был удален TODO: Добавить поддержку форматтера пользовательских голов | TODO: Отложено | Причина:
Выполнен - 💚.

E1m0:
🧑‍💻 | Из-за сраного решейда, у меня плагин весит как самолет, при чем я даже указатели дал.. Я хочу сделать мультик так
еще и с кучей возможностей, но когда без решейда он весит 133кб меня прямо гордость берет, либо отказ от JDBI - либо
терпеть 9мб, я выбрал терпеть, потом собрать статистику и - на удешевление.
